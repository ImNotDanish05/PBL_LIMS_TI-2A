...
Route::middleware(['auth', 'role:manager'])
    ...
    ->as('manager.')
    ->group(function () {
        Route::get('/', [\App\Http\Controllers\ManagerController::class, 'dashboard'])->name('index');

        Route::prefix('report-validation')->as('report.validation.')->group(function () {
            Route::get('/', [\App\Http\Controllers\ManagerController::class, 'reportValidationIndex'])->name('index');
            Route::get('/{id}', [\App\Http\Controllers\ManagerController::class, 'reportValidationShow'])->name('show');
        });

        Route::prefix('tests')->as('tests.')->group(function () {
            Route::get('/categories', [\App\Http\Controllers\ManagerController::class, 'testCategories'])->name('categories');
            Route::get('/parameters', [\App\Http\Controllers\ManagerController::class, 'testParameters'])->name('parameters');
            Route::get('/methods', [\App\Http\Controllers\ManagerController::class, 'testMethods'])->name('methods');
            Route::get('/units', [\App\Http\Controllers\ManagerController::class, 'testUnits'])->name('units');
            Route::get('/references', [\App\Http\Controllers\ManagerController::class, 'testReferences'])->name('references');
        });

        Route::inertia('/reports', 'manager/reports/index')->name('reports');

        Route::prefix('orders')->as('orders.')->group(function () {
            Route::get('/', [\App\Http\Controllers\ManagerController::class, 'ordersIndex'])->name('index');
            Route::get('/{id}', [\App\Http\Controllers\ManagerController::class, 'ordersShow'])->name('show');
        });

        Route::get('/users', [\App\Http\Controllers\ManagerController::class, 'usersIndex'])->name('users');
    });
